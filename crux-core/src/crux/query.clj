...
(declare pred-args-spec aggregate)
...
(defn- pred-constraint? [x]
  (contains? (methods pred-args-spec) x))
...
(def ^:private pred->built-in-range-pred-source  {< '(comp neg? compare)
                                                  <= '(comp not pos? compare)
                                                  > '(comp pos? compare)
                                                  >= '(comp not neg? compare)
                                                  = '=})
           ...
           (case type
             ...
             :range (let [[order clause] (first clause)
                      ...
                      (if (and (not= :sym-sym order)
                               (not (logic-var? sym)))
                        {:pred [{:pred {:args [sym val]}
                                 :source `(~(get pred->built-in-range-pred-source (var-get (resolve op))) ~sym ~val)}]}
                        {:range [clause]}))
...
(defn- triple-join-order [triple-clauses in-vars range-vars stats]
  (let [var->frequency (->> (concat (map :e triple-clauses)
        ...
        var-triple-clauses (remove (set literal-clauses) triple-clauses)
        join-order (loop [join-order (concat literal-join-order self-join-vars)
                          clauses var-triple-clauses]
...
(defn- triple-pred-clauses [triple-clauses range-vars known-vars stats]
  (let [[triple-join-order literal-clauses var-triple-clauses] (triple-join-order triple-clauses
                                                                                  known-vars
                                                                                  range-vars
                                                                                  stats)
        literal-preds (for [literal-clause literal-clauses]
                        (build-literal-triple-pred-clause literal-clause))]
    (first
     (reduce
      (fn [[acc known-vars clauses] var]
        (let [triple-preds (for [{:keys [e a v] :as clause} clauses
                                 :when (or (= e var) (= v var))]
                             (build-var-triple-pred-clause clause known-vars var))
             ...
             (let [return-binding (first return-binding)
               ...
               [(assoc flat-pred :return [:scalar collection-var])
                {:pred {:args ['$ collection-var]}
                 ...
                 :return return}])
             [(assoc flat-pred :predicate (keyword (gensym (str "predicate_"))))])))
       (reduce into [])))

(defn- build-var-range-constraint-sources [range-clauses vars-in-join-order]
    ...
    (merge (->> (for [[var clauses] (group-by :sym range-clauses)
           ...
           (->> (for [{:keys [op sym-a sym-b] :as clause} range-clauses
                      :when (and (logic-var? sym-a)
                                 (logic-var? sym-b))
                      :let [sym+index [[sym-a (get var->index sym-a)]
                                       [sym-b (get var->index sym-b)]]
...
(defn- codegen-sub-query [flat-pred-clauses range-clauses rules compiled-find vars-in-join-order]
  (let [var->pred-clauses (group-by #(or (:predicate %)
                                    ...
                                    flat-pred-clauses)
        var->range-constraint-sources (build-var-range-constraint-sources range-clauses vars-in-join-order)
        ...
        clause->source (fn [{:keys [source] :as clause} return-type]
                         (if (= 'q (first source))
                           (let [[_ q & args] source
                                 q (normalize-query (maybe-unquote q))
                                 q (cond-> q
                                     (seq rules) (update :rules (comp vec concat) rules)
                                     (= :predicate return-type) (assoc :limit 1))]
    ...
    `(fn [~'$ ~in-args-sym]
       (let [~'$ (assoc ~'$ :in-args ~in-args-sym :encode-value-fn (partial db/encode-value (:index-snapshot ~'$)))
             ...
             ~'q (fn [query# & args#]
                   (let [query# (crux.query/maybe-unquote query#)
                     ...
                     (if-let [cached-result# (when cache-key#
                                               (get-in ~'$ [:recursion-table* cache-key#]))]
                       cached-result#
                       (let [~'$ (if cache-key#
             ...
             ~'get-attr (fn
                          ([e# a#]
                           (->> (db/aev ~index-snapshot-sym a# e# nil  ~entity-resolver-fn-sym)
                                (map (partial db/decode-value ~index-snapshot-sym))
                                (not-empty)))
                          ([e# a# not-found#]
                           (if-let [vs# (not-empty (db/aev ~index-snapshot-sym a# e# nil ~entity-resolver-fn-sym))]
                             (map (partial db/decode-value ~index-snapshot-sym) vs#)
                             [not-found#])))
             ~'== =
             ~'!= not=]
         (for [loop# [nil]
               ~@(->> (for [var vars-in-join-order
                        ...
                        (case return-type
                          :scalar
                          `[:let ~@(for [clause clauses]
                                     [var (clause->source clause :scalar)])]
                          :collection
                          `[~var (idx/idx->seq
                                  (idx/wrap-with-range-constraints
                                   (idx/new-unary-join-virtual-index
                                    [~@(map #(clause->source % :collection) clauses)])
                                   ~(when-let [cs (get var->range-constraint-sources var)]
                                      `(comp ~@cs))))
                            :let [~var (db/decode-value ~index-snapshot-sym ~var)]]
                          `[:when (and ~@(for [clause clauses]
                                           (clause->source clause :predicate)))]))
                      (reduce into []))]
           [~@find-vars])))))
...
(defn- compile-sub-query [{:keys [find where in rules full-results?]} stats]
...
(defn- prepare-sub-query [{:keys [index-snapshot query-cache] :as db} query stats]
  (let [query (select-keys query [:find :where :in :rules :full-results?])
        compiled-query (lru/compute-if-absent
                        query-cache
                        query
                        identity
                        (fn [_]
                          (compile-sub-query query stats)))]
    (select-keys compiled-query [:query-fn :compiled-find])))
